<template>
  <div class="home"> 
    ...
    <div class="block main marginTop wow slideInLeft">
      <!-- 小标题组件 -->
      <title-box :title="title" class="title-box wow slideInLeft" data-wow-delay="0.4s"></title-box>
      <!-- 文章列表组件 -->
...
export default {
   ...
   */ 
   ThumbRank(array) {
    ...
    GetAllArticle(){
     this.$http.get('/api/article/typeList').then(res => {
        if(res.data.code === 0){
          ...
          // 获取3篇文章
          this.AllArticle = res.data.data.slice(3*(this.currentPage-1),this.pageSize+3*(this.currentPage-1))

          this.total = res.data.data.length
          // 截取最新的6篇给轮播图展示  .concat() 先将文章数组复制出一个新数组再进行反转，如果使用.reverse(),则会改变文章数组，而不产生新数组
          this.LbtArticle = res.data.data.concat().reverse().slice(0,5)
            // 截取一部分给轮播图右边的盒子
    ...
    // 获取当前页码的数据 用前端处理了分页，没有去请求后端数据，因为前面一次性请求完了
    handleCurrentChange(CurrentPage){

      this.currentPage = CurrentPage
      // console.log(CurrentPage)
      // 再次调用 GetAllArticle 实现分页
      // 主要思路是先提前获取所有文章列表，然后让前端进行切割实现分页效果。
      this.GetAllArticle()
    }
